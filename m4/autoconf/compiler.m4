...
AC_DEFUN([AC_PROG_CC_WORKS], [
    AC_MSG_CHECKING_CACHE_IFELSE([whether the C compiler works], [
        tmp1=`as_mktemp ".c"`
        tmp1=`basename "$tmp1"`
        cat > "$tmp1" <<AS_EOF
#include <stdio.h>
...
AC_DEFUN([AC_PROG_CC_C_O], [
    AC_MSG_CHECKING_CACHE_IFELSE([whether the C compiler understands both -c and -o together], [
        tmp1=`as_mktemp ".c"`
        tmp2=`as_mktemp ".o"`
        cat > "$tmp1" <<AS_EOF
int
